...
begin
  S := 'Delphi';
 i:= Length (S);  //Zwróci 6
 if Length (S)= (6) then raise Exception.Create('wyskoczy³a szóstka!')
  else
...
begin
  ...
  if q = 17   then   raise ELowError.Create('Coœ strasznego! Zakoñcz aplikacjê!')
     else
...
begin
  ...
  MojeImie := 'Gerwazy';
  Wiek := 17;
  //poprawka na potrzeby git
  ShowMessage(Format('Witaj %s, nazywam siê %s i mam %d lat', [ImieKolegi, MojeImie, Wiek]));
end;
...
begin
  ...
  ShowMessage(RightStr(S, 11));  //Zobaczymy "Cztery Piêæ"
end;
